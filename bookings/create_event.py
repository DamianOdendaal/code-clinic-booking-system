...
def get_user_event_input():
    ...
    try:
        ...
        student_file_name = creator.split('@')[0]
        ...
        with open(student_file_name+'.json','w') as fp:
            json.dump(studentDocument, fp)

        # f = open(student_file_name+'.json')
        # data = json.load(f)
